    ...
    <div class="app-container">
        <el-row :gutter="20" style="margin-left:-10%;">
            ...
            <el-col :span="20" :xs="24" class="tooltup" style="width:100%;">
                ...
                <el-table border :header-cell-style="headClassWSm" :row-style="{height: '3px'}" :cell-style="{padding: '2px'}" v-loading="loading" :data="userList" height="470"
...
import { PurchasereturnordersAdd, SkuBarcodeLists, PurchaseinboundEdit, PurchasereturnorderRemove, Purchaseinboundsho, PurchaseinBoundshf, PurchaseinboundShtt, PurchaseinboundSht, SupplierList, GoodsList, StoreList, StoreSkuList } from "@/api/Warehousemanagement/WarehouseSummary";
import * as req from "@/api/Warehousemanagement/WarehouseSummary";
...
export default {
    ...
    methods: {
        ...
        PurchaseinboundShenpi01(row) {
            ...
            console.log(row.cbpc01, 8888);
            this.$modal.confirm('是否确定盘点完成为"' + JSON.stringify(this.idss) + '"的数据项？').then(() => {
        ...
        PurchaseinboundBiaojiWancheng(row) {
            console.log(row.cbpc01, 8888);
            this.$modal.confirm('是否确定标记为"' + row.cbpc01 + '"的数据项？').then(() => {
        ...
        PurchaseinboundBiaojiWancheng01(row) {
            let userIds = this.shenpiids.length > 0 ? this.shenpiids : row
            console.log(row.cbpc01, 8888);
            this.$modal.confirm('是否确定标记为"' + JSON.stringify(this.idss) + '"的数据项？').then(() => {
                userIds.forEach((item) => {
